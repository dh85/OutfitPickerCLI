...
struct CLIExtensionsTests {
    ...
    @Test func getAllOutfitStatesReturnsAllCategories() async throws {
        ...
        let categoryInfos = [
            CategoryInfo(
                category: CategoryReference(name: "casual", path: casualURL.path(percentEncoded: false)),
                state: .hasOutfits,
                outfitCount: 1
            ),
            CategoryInfo(
                category: CategoryReference(name: "formal", path: formalURL.path(percentEncoded: false)),
                state: .hasOutfits,
                outfitCount: 1
            )
        ]
        ...
        let outfitsByPath = [
            casualURL.path(percentEncoded: false): [FileEntry(filePath: "\(casualURL.path(percentEncoded: false))/shirt.avatar")],
            formalURL.path(percentEncoded: false): [FileEntry(filePath: "\(formalURL.path(percentEncoded: false))/suit.avatar")]
        ]
    ...
    @Test func isWornDelegatesToIsOutfitWorn() async throws {
        ...
        let outfit = makeOutfitReference(root: "/Users/test/Outfits", category: "casual", fileName: "shirt.avatar")
